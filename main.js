...
function getChromecasts(callback){
  ...
  exec(path.join('/usr/local/bin/node ',__dirname,'/node_modules/chromecast-osx-audio/bin/chromecast.js -j'), function (error, stdout, stderr) {
    if(stdout){
    ...
    for(var i in chromecasts) {
      ...
      castmenu.append(new MenuItem({
        ...
        click: function(current){
          ...
          chromecastProcess = exec(path.join('/usr/local/bin/node ',__dirname,'/node_modules/chromecast-osx-audio/bin/chromecast.js -n '+caption+' -p '+port+' -d "'+current.sublabel+'"'), function (err, stdout, stderr){
            if (err) {
